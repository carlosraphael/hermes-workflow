...
"""Single-source command registry (issue #15), now homed in ``cli.py`` (issue #16).

``cli.COMMANDS`` is the one place that enumerates the workflow commands; the CLI
...
def test_cli_subparsers_are_derived_from_commands():
    ...
    assert _subparser_names(parser) == {cmd.cli_name for cmd in cli.COMMANDS}
...
def test_dispatch_routes_exactly_the_commands():
    ...
    assert set(cli._COMMANDS_BY_CLI) == {cmd.cli_name for cmd in cli.COMMANDS}
    for cmd in cli.COMMANDS:
        assert cli._COMMANDS_BY_CLI[cmd.cli_name] is cmd
...
def test_dispatch_unknown_subcommand_returns_flat_error():
    ...
    result = cli._dispatch_ns(object(), argparse.Namespace(wf_cmd="nope"))
